...
@section('title', __('Додај тура'))

@section('content')
<div class="bg-white rounded shadow p-6 max-w-2xl">
    <h1 class="text-3xl font-bold mb-6">{{ __('Нова тура') }}</h1>
    ...
    <form method="POST" action="{{ route('tours.store') }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="number" name="cost_per_km" value="{{ old('cost_per_km') }}" step="0.01" min="0"
        ...
        <div>
            ...
            <input type="text" name="start_location" value="{{ old('start_location') }}" required
                ...
                placeholder="нпр. Скопје" id="start-location">
        ...
        <div>
            ...
            <input type="number" name="total_km" value="{{ old('total_km', 0) }}" min="0"
        ...
        <div>
            ...
            <div class="w-full border border-gray-300 rounded px-3 py-2 bg-gray-50 text-gray-600 font-bold">
                <span id="tour-cost">0.00</span> ЕУР
                <small class="text-gray-500 block" id="cost-per-km-display">(0.00 ЕУР/км)</small>
            </div>
        </div>

        <script>
            const truckSelect = document.getElementById('truck-select');
            const trucksDataRaw = truckSelect.dataset.trucks || '{}';
            
            // Parse trucks data and ensure cost_per_km is a number
            let trucksData = {};
            try {
                const parsed = JSON.parse(trucksDataRaw);
                Object.keys(parsed).forEach(key => {
                    trucksData[key] = {
                        cost_per_km: parseFloat(parsed[key].cost_per_km) || 0.50
                    };
                });
            } catch (e) {
                console.error('Error parsing trucks data:', e);
            }
            ...
            function updateCost() {
                ...
                let truckCostPerKm = trucksData[truckId]?.cost_per_km || 0.50;
                const costPerKm = !isNaN(customCostPerKm) ? customCostPerKm : parseFloat(truckCostPerKm);
            ...
            function updateDefaultDisplay() {
                const truckId = truckSelect.value;
                let truckCostPerKm = trucksData[truckId]?.cost_per_km || 0.50;
                defaultCostDisplay.textContent = 'Камионов default: ' + parseFloat(truckCostPerKm).toFixed(2) + ' ЕУР/км';
            }
            ...
            calculateDistanceBtn.addEventListener('click', async function() {
                ...
                if (!startLocation || !endLocation) {
                    distanceMessage.textContent = '❌ Внеси почетна и крајна локација';
                    distanceMessage.className = 'text-red-600 block mt-1';
                    return;
                }

                calculateDistanceBtn.disabled = true;
                calculateDistanceBtn.textContent = '⏳ Се пресметува...';
                distanceMessage.textContent = '';

                try {
                    const response = await fetch('{{ route("calculate-distance") }}', {
                        method: 'POST',
                        headers: {
                    ...
                    if (data.success) {
                        kmInput.value = data.distance;
                        updateCost();
                        distanceMessage.textContent = `✓ ${data.distance_text}`;
                        if (data.note) {
                            distanceMessage.textContent += ` • ${data.note}`;
                        }
                        distanceMessage.className = 'text-green-600 block mt-1 text-sm';
                    } else {
                        let errorMsg = data.error || 'Постои грешка при пресметување';
                        distanceMessage.textContent = `❌ ${errorMsg}`;
                        distanceMessage.innerHTML += `<br><small class="text-gray-600">Совет: Пробај да напишеш полно име на град (нпр. "Skopje, Macedonia")</small>`;
                        distanceMessage.className = 'text-red-600 block mt-1';
                    }
                } catch (error) {
                    distanceMessage.textContent = `❌ Постои грешка: ${error.message}`;
                    distanceMessage.className = 'text-red-600 block mt-1';
                } finally {
                    calculateDistanceBtn.disabled = false;
                    calculateDistanceBtn.textContent = '📍 Пресметај растојание';
                }
            });
        </script>
        ...
        <div>
            ...
            <select name="status" class="w-full border rounded px-3 py-2">
                <option value="planned" selected>{{ __('Планирано') }}</option>
                <option value="in-progress">{{ __('Во тек') }}</option>
                <option value="completed">{{ __('Завршено') }}</option>
            </select>
        </div>
